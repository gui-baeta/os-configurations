...
def process_images(source_dir, target_parent_dir, timezone_info=None):
    ...
    for burst in bursts:
        for seq_num, (img_path, img_time) in enumerate(burst, 1):
            ...
            if main_target_path.exists():
                print(f"? {main_filename}")
                skipped_files.append((str(img_path), str(main_target_path)))
                # Still need to create hardlinks in sorted folders if they don't exist
                main_exists = True
            else:
    ...
    if skipped_files:
        print("\n" + "="*60)
        print("SUMMARY: Files that already existed and were skipped:")
        print("="*60)
        for source_file, target_file in skipped_files:
